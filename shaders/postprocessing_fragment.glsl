...
vec3 getLookingDirection() {
    ...
    return normalize(worldSpaceCoordinates.xyz - uCameraPosition);
}


vec3 starsColor(vec3 color, vec3 lookingDirection) {
    if (lookingDirection.y < 0.0 || uCameraPosition.y < 0.0) {
        return color;
    }

    float atmosphereIntensity = dot(color, vec3(0.299, 0.587, 0.114));

    float stars = snoise(lookingDirection * uStarsDensity) > uStarsThreshold ? 1.0 : 0.0;
...
vec3 atmosphereColor(vec3 sunPosition, vec3 horizonColor) {
    ...
    sky = starsColor(sky, lookingDirection);
